    ...
    ILogger<SupabaseAdminBootstrap> logger)
{
    public async Task<ApplicationUser?> ResolveUserAsync(string accessToken)
    {
        ...
        if (!Guid.TryParse(verified.Id, out _) || verified.EmailConfirmedAt == null ||
            string.IsNullOrWhiteSpace(verified.Email))
            throw new SupabaseAuthException("A verified email is required before signing in.");
